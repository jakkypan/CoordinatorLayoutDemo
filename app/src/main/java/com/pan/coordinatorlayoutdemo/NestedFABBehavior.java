...
public class NestedFABBehavior extends FloatingActionButton.Behavior {
    ...
    public void onNestedScroll(@NonNull CoordinatorLayout coordinatorLayout, @NonNull FloatingActionButton child, @NonNull View target, int dxConsumed, int dyConsumed, int dxUnconsumed, int dyUnconsumed, int type) {
        ...
        if (dyConsumed > 0 && child.getVisibility() == View.VISIBLE) {
            //系统FAB控件提供的隐藏动画
            try {
                Method mMethod = child.getClass().getDeclaredMethod("hide",
                        FloatingActionButton.OnVisibilityChangedListener.class, boolean.class);
                mMethod.setAccessible(true);
                mMethod.invoke(child, null, false);
            } catch (Throwable e) {
                e.printStackTrace();
            }
